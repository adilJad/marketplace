...
marketplace.run(function($ionicPlatform, $log, $rootScope, MarketplaceStorage) {
  $ionicPlatform.ready(function() {
    ...
    //Vouchers
    MarketplaceStorage.executeQuery('INSERT OR IGNORE INTO Vouchers(title, shop, reduction, duration, quantity, creator_id) VALUES(?, ?, ?, ?, ?, ?)', ["Get ready for school!", "The cool library", 30, 48, 20, 2]);
    MarketplaceStorage.executeQuery('INSERT OR IGNORE INTO Vouchers(title, shop, reduction, duration, quantity, creator_id) VALUES(?, ?, ?, ?, ?, ?)', ["Treat yo self this spring!", "Hipster clothing & accessories", 50, 48, 25, 3]);
    MarketplaceStorage.executeQuery('INSERT OR IGNORE INTO Vouchers(title, shop, reduction, duration, quantity, creator_id) VALUES(?, ?, ?, ?, ?, ?)', ["Eat you weight in Cake!", "Bake n' Cake", 25, 48, 25, 1]);
    MarketplaceStorage.executeQuery('INSERT OR IGNORE INTO Vouchers(title, shop, reduction, duration, quantity, creator_id) VALUES(?, ?, ?, ?, ?, ?)', ["Get you mom the gift she deserves for Mather's Day", "Jewelery store", 25, 48, 25, 1]);
    MarketplaceStorage.executeQuery('INSERT OR IGNORE INTO Vouchers(title, shop, reduction, duration, quantity, creator_id) VALUES(?, ?, ?, ?, ?, ?)', ["No outfit is complete without nice shoes", "Shoe Store", 40, 48, 25, 5]);
    MarketplaceStorage.executeQuery('INSERT OR IGNORE INTO Vouchers(title, shop, reduction, duration, quantity, creator_id) VALUES(?, ?, ?, ?, ?, ?)', ["Get smart with a smart watch", "Gadget store", 35, 48, 25, 4]);
    MarketplaceStorage.executeQuery('INSERT OR IGNORE INTO Vouchers(title, shop, reduction, duration, quantity, creator_id) VALUES(?, ?, ?, ?, ?, ?)', ["Go see the world like you always dreamed", "Travel agency", 25, 30, 15, 3]);
  })
})
